...
# done: (Task P1.1) Add fact_text parameter
...
def create_fact(fact_text) -> Fact:
    ...
    with provider.cursor() as cur:
        ...
        query = "INSERT INTO facts(fact) VALUES ('" + fact_text + "') RETURNING id;"
        cur.execute(query) # SQL query goes here
        ...
        if not fact_data:
            ...
            return Fact(
                id=fact_data.id,
                fact= fact_data.fact,   # done: (Task P1.1) Create and return a Fact object using the data from the database result
