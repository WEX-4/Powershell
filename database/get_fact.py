...
def get_fact() -> Fact:
    ...
    with provider.cursor() as cur:
        ...
        # DONE: (Task P0.1) Write SQL query to select a random fact from the database
        cur.execute("SELECT id, fact FROM facts ORDER BY RANDOM() LIMIT 1;") # SQL query goes here
        ...
        # DONE: (Task P0.1) Create and return a Fact object using the data from the database result
        # TODO: (Task P3.1) Add the likes and dislikes counts to the Fact object
        # TODO: (Task P4.2) Add the category information
        return Fact(
            id=fact_data.id,
            fact= fact_data.fact,     # DONE: (Task P0.1) Add parsed fact text
